...
public class Vector implements IVector, Comparable<Vector> {
    ...
    public static void main(String[] args) {
        ...
        Arrays.sort(Vs);
        List<Vector> vek = new ArrayList();
        for (int i = 0; i < Vs.length; i++) {
            vek.add(Vs[i]);
        }
        vek.sort(new CompareVector());

        LinkedList<Vector> setVec = new LinkedList<>();
        for (int i = 0; i < Vs.length; i++) {
            setVec.add(Vs[i]);
        }


        setVec.sort(new CompareVector());


        for (int i = 0; i < Vs.length; i++) {
            System.out.println(Vs[i].x + ", " + Vs[i].y );
            System.out.println(setVec.get(i).getX() + ", " + setVec.get(i).getY());
        }
    }
}
